...
class Discriminator(nn.Module):
    def __init__(self, image_dimension=784, hidden_dimension=128):
        ...
        self.disc = nn.Sequential(
            ...
            self.discriminator_block(hidden_dimension * 4, hidden_dimension * 2),
...
for epoch in range(n_epochs):
    ...
    print(f"Epoch {epoch+1}/{n_epochs} - Avg Gen Loss: {avg_gen_loss:.4f}, Avg Disc Loss: {avg_disc_loss:.4f}")
...
gen_signature = infer_signature(
    gen_input_example.cpu().numpy(), 
    gen_output.cpu().numpy()
)
...
disc_signature = infer_signature(
    disc_input_example.cpu().numpy(), 
    disc_output.cpu().numpy()
)
